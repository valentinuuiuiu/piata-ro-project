...
cd ./mcp_agents/sql_agent && python3 main.py > sql_agent.log 2>&1 &

# Start Marketing agent in background
echo "Starting Marketing Agent on port 8001..."
cd ./mcp_agents/marketing_agent && python3 main.py > marketing_agent.log 2>&1 &

echo "MCP agents started. Check logs for details."
echo "SQL Agent log: ./mcp_agents/sql_agent/sql_agent.log"
echo "Marketing Agent log: ./mcp_agents/marketing_agent/marketing_agent.log"
